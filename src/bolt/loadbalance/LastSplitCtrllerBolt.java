...
import tool.loadbalance.PlanGeneratorSplitLast;
 ...
 */
public class LastSplitCtrllerBolt extends BaseRichBolt{
    ...
    public void prepare(Map map, TopologyContext topologyContext, OutputCollector outputCollector) {
        ...
        bwTable = FileProcess.getWriter("Experiment/tableLast.txt");
        bwLoad=FileProcess.getWriter("Experiment/loadLast.txt");
    }
    ...
    public void execute(Tuple tuple) {
        ...
        if(signal.equalsIgnoreCase(SystemParameters.JOINER_MIG_END)){
            ...
            if(curEndNum== PlanGeneratorSplitLast.endCheckInfo.size()){
                System.out.println("收到所有DBolt迁移结束信号\n\n");
                curEndNum=0;
                PlanGeneratorSplitLast.reset();
                for(int i=0;i<usedDBoltNum;i++){
                    usedDBolt.add(i);
                }
                jedis.set("usedDBolt".getBytes(),SerializeUtil.serialize(usedDBolt));
            ...
            if(usedDBolt.contains(dBoltIndex)){
                ...
                PlanGeneratorSplitLast.update(dBoltIndex,taskLoad,loadDetails);
                usedDBolt.remove(dBoltIndex);
                jedis.set("usedDBolt".getBytes(),SerializeUtil.serialize(usedDBolt));
                if(usedDBolt.isEmpty()){
                    flag=false;
                    PlanGeneratorSplitLast.writeLoadToFile(bwLoad,reportTime-1);
                    boolean adjust=PlanGeneratorSplitLast.makeplanMNRT();
                    if(adjust==false){
                        ...
                        writeToRedis(PlanGeneratorSplitLast.routeTable,"routetable");
                        writeToRedis(PlanGeneratorSplitLast.endCheckInfo,"endcheckinfo");
                        PlanGeneratorSplitLast.writeTableToFile(bwTable,reportTime-1);
                        PlanGeneratorSplitLast.writeMigratePlan(bwTable);
                        PlanGeneratorSplitLast.writeCheckInfo(bwTable);
                        //通知UBolt更新路由表
                        for(int i=0;i<uBoltPhysicalIDs.size();i++){
                        ...
                        //通知DBolt，进行数据迁移
                        for(Map.Entry<Integer,ArrayList<MigrationItem>> mpi:PlanGeneratorSplitLast.migrationPlan.entrySet()){
