...
import tool.loadbalance.PlanGeneratorSplitFirst;
 ...
 */
public class FirstSplitCtrllerBolt extends BaseRichBolt{
    ...
    public void prepare(Map map, TopologyContext topologyContext, OutputCollector outputCollector) {
        ...
        bwTable = FileProcess.getWriter("Experiment/tableFirst.txt");
        bwLoad=FileProcess.getWriter("Experiment/loadFirst.txt");
    }
    ...
    public void execute(Tuple tuple) {
        ...
        if(signal.equalsIgnoreCase(SystemParameters.JOINER_MIG_END)){
            ...
            if(curEndNum== PlanGeneratorSplitFirst.endCheckInfo.size()){
                System.out.println("收到所有DBolt迁移结束信号");
                curEndNum=0;
                PlanGeneratorSplitFirst.reset();
                for(int i=0;i<usedDBoltNum;i++){
                    usedDBolt.add(i);
                }
                jedis.set("usedDBolt".getBytes(),SerializeUtil.serialize(usedDBolt));
            ...
            if(usedDBolt.contains(dBoltIndex)){
                ...
                PlanGeneratorSplitFirst.update(dBoltIndex,taskLoad,loadDetails);
                usedDBolt.remove(dBoltIndex);
                jedis.set("usedDBolt".getBytes(),SerializeUtil.serialize(usedDBolt));
                if(usedDBolt.isEmpty()){
                    flag=false;
                    PlanGeneratorSplitFirst.writeLoadToFile(bwLoad,reportTime-1);
                    boolean adjust=PlanGeneratorSplitFirst.makeplanMNRT();
                    if(adjust==false){
                        ...
                        writeToRedis(PlanGeneratorSplitFirst.routeTable,"routetable");
                        writeToRedis(PlanGeneratorSplitFirst.endCheckInfo,"endcheckinfo");
                        PlanGeneratorSplitFirst.writeTableToFile(bwTable,reportTime-1);
                        PlanGeneratorSplitFirst.writeMigratePlan(bwTable);
                        PlanGeneratorSplitFirst.writeCheckInfo(bwTable);
                        //通知UBolt更新路由表
                        for(int i=0;i<uBoltPhysicalIDs.size();i++){
                        ...
                        //通知DBolt，进行数据迁移
                        for(Map.Entry<Integer,ArrayList<MigrationItem>> mpi:PlanGeneratorSplitFirst.migrationPlan.entrySet()){
